...
#include "nr3.h"
#include "ran.h"
template <class T> struct Amebsa {
  T &funk;
  const Doub ftol;
  Ranq1 ran;
  Doub yb;
  ...
  Amebsa(VecDoub_I &point, const Doub del, T &funkk, const Doub ftoll)
      : funk(funkk), ftol(ftoll), ran(1234), yb(numeric_limits<Doub>::max()),
        ndim(point.size()), pb(ndim), mpts(ndim + 1), y(mpts), p(mpts, ndim) {
  ...
  Amebsa(MatDoub_I &pp, T &funkk, const Doub ftoll)
      : funk(funkk), ftol(ftoll), ran(1234), yb(numeric_limits<Doub>::max()),
  ...
  Doub amotsa(MatDoub_IO &p, VecDoub_O &y, VecDoub_IO &psum, const Int ihi,
              Doub &yhi, const Doub fac) {
    VecDoub ptry(ndim);
