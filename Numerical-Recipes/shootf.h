...
template <class L1, class L2, class R, class S> struct Shootf {
  ...
  Shootf(Int nvarr, Int nn2, Doub xx1, Doub xx2, Doub xxf, L1 &loadd1,
         L2 &loadd2, R &dd, S &scoree)
      : nvar(nvarr), n2(nn2), x1(xx1), x2(xx2), xf(xxf), load1(loadd1),
        load2(loadd2), d(dd), score(scoree), atol(1.0e-14), rtol(atol),
        hmin(0.0), y(nvar), f1(nvar), f2(nvar) {}
