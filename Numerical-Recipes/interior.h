...
Int intpt(const NRsparseMat &a, VecDoub_I &b, VecDoub_I &c, VecDoub_O &x) {
  ...
  Doub normrp_old = BIG;
  Doub normrd_old = BIG;
  cout << setw(4) << "iter" << setw(12) << "Primal obj." << setw(9) << "||r_p||"
       << setw(13) << "Dual obj." << setw(11) << "||r_d||" << setw(13)
       << "duality gap" << setw(16) << "normalized gap" << endl;
  ...
  for (iter = 0; iter < MAXITS; iter++) {
    ...
    cout << setw(3) << iter << setw(12) << primal_obj << setw(12) << normrp
         << setw(12) << dual_obj << setw(12) << normrd << setw(12) << gamma
         << setw(12) << gamma_norm << endl;
...
NRldl::NRldl(NRsparseMat &adat)
    : n(adat.ncols), nz(adat.nvals), Ap(&adat.col_ptr[0]), Ai(&adat.row_ind[0]),
      Ax(&adat.val[0]), PP(n), PPinv(n), PPattern(n), LLnz(n), LLp(n + 1),
      PParent(n), FFlag(n), YY(n), DD(n), Y(&YY[0]), D(&DD[0]), P(&PP[0]),
      Pinv(&PPinv[0]), Pattern(&PPattern[0]), Lnz(&LLnz[0]), Lp(&LLp[0]),
      Parent(&PParent[0]), Flag(&FFlag[0]) {}
...
void NRldl::factorize() {
  ...
  /* -------------------------------------------------------------- */
  Int dd = ldl_numeric(n, Ap, Ai, Ax, Lp, Parent, Lnz, Li, Lx, D, Y, Flag,
                       Pattern, P, Pinv);
  if (dd != n)
