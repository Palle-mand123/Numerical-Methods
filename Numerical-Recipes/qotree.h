...
template <class elT, Int DIM> Box<DIM> Qotree<elT, DIM>::qobox(Int k) {
  Int j, kb;
  Point<DIM> plo, phi;
  Doub offset[DIM];
  Doub del = 1.0;
...
template <class elT, Int DIM> Int Qotree<elT, DIM>::qowhichbox(elT tobj) {
  Int p, k, kl, kr, ks = 1;
  for (p = 2; p <= maxd; p++) {
    kl = QO * ks - QL;
    kr = kl + QO - 1;
...
template <class elT, Int DIM> Int Qotree<elT, DIM>::qostore(elT tobj) {
  Int k, ks, kks, km;
...
template <class elT, Int DIM> Int Qotree<elT, DIM>::qoerase(elT tobj) {
  Int k, ks, kks, km;
  Int *ppop;
  ks = kks = qowhichbox(tobj);
