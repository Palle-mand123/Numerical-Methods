...
Solvde::Solvde(const Int itmaxx, const Doub convv, const Doub slowcc,
    ...
    : itmax(itmaxx), conv(convv), slowc(slowcc), scalv(scalvv), indexv(indexvv),
      nb(nbb), y(yy), difeq(difeqq), ne(y.nrows()), m(y.ncols()), kmax(ne),
      ermax(ne), c(ne, ne - nb + 1, m + 1), s(ne, 2 * ne + 1) {
  Int jv, k, nvars = ne * m;
  Int k1 = 0, k2 = m;
