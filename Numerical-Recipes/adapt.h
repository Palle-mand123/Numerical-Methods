...
struct Adapt {
  ...
  Adapt(Doub tol);
  template <class T> Doub integrate(T &func, const Doub a, const Doub b);
  template <class T>
...
template <class T> Doub Adapt::integrate(T &func, const Doub a, const Doub b) {
...
Doub Adapt::adaptlob(T &func, const Doub a, const Doub b, const Doub fa,
  ...
  i1 = h / 1470.0 *
       (77.0 * (fa + fb) + 432.0 * (fmll + fmrr) + 625.0 * (fml + fmr) +
        672.0 * fm);
...
const Doub Adapt::x[12] = {0,   -x1, -alpha, -x2, -beta, -x3,
                           0.0, x3,  beta,   x2,  alpha, x1};
#endif // _ADAPT_H_
