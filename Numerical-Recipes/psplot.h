...
struct PSpage {
  ...
  void putctext(char *text, Doub x, Doub y, Doub rot = 0.0) {
    fprintf(PLT, "gsave %g %g translate %g rotate 0 0 mt (%s) ", x, y, rot,
            text);
  ...
  void polyline(VecDoub &x, VecDoub &y, Int close = 0, Int fill = 0,
                Int clip = 0) {
...
struct PSplot : PSpage {
  ...
  PSplot(PSpage &page, Doub ppll, Doub ppur, Doub qqll, Doub qqur)
      : pll(ppll), qll(qqll), pur(ppur), qur(qqur), xll(ppll), yll(qqll),
        xur(ppur), yur(qqur), xbox(4), ybox(4), majticsz(8.), minticsz(4.) {
  ...
  void scales(Doub xmajd, Doub xmind, Doub ymajd, Doub ymind, Int dox = 2,
              Int doy = 2, Int doxx = 1, Int doyy = 1) {
    char str[128];
    Doub x, y, xlo, ylo;
    if (dox || doxx) {
