...
struct Svmpolykernel : Svmgenkernel {
  ...
  Svmpolykernel(MatDoub_I &ddata, VecDoub_I &yy, Doub aa, Doub bb, Doub dd)
      : Svmgenkernel(yy, ddata), n(data.ncols()), a(aa), b(bb), d(dd) {
    fill();
  }
  Doub kernel(const Doub *xi, const Doub *xj) {
    Doub dott = 0.;
...
struct Svmgausskernel : Svmgenkernel {
  ...
  Svmgausskernel(MatDoub_I &ddata, VecDoub_I &yy, Doub ssigma)
      : Svmgenkernel(yy, ddata), n(data.ncols()), sigma(ssigma) {
    fill();
  }
  Doub kernel(const Doub *xi, const Doub *xj) {
    Doub dott = 0.;
...
struct Svm {
  ...
  Bool alphinit;
  Doub dalph;
  Svm(Svmgenkernel &inker)
      : gker(inker), m(gker.y.size()), alph(m), alphold(m), ran(21),
        alphinit(false) {}
  Doub relax(Doub lambda, Doub om) {
    ...
    niter =
        MAX(Int(0.5 * (m + 1.0) * (m - fnz + 1.0) / (SQR(fub - fnz + 1.0))), 1);
    for (iter = 0; iter < niter; iter++) {
      for (jj = fnz; jj < fub; jj++) {
        ...
        alph[j] =
            alph[j] - (om / (gker.ker[j][j] + 1.)) * (gker.y[j] * sum - 1.);
        alph[j] = MAX(0., MIN(lambda, alph[j]));
      }
    }
    dalph = 0.;
