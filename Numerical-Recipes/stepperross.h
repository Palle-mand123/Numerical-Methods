...
struct Ross_constants {
  static const Doub c2, c3, c4, bet2p, bet3p, bet4p, d1, d2, d3, d4, a21, a31,
      a32, a41, a42, a43, a51, a52, a53, a54, c21, c31, c32, c41, c42, c43, c51,
      c52, c53, c54, c61, c62, c63, c64, c65, gam, d21, d22, d23, d24, d25, d31,
      d32, d33, d34, d35;
};
...
template <class D> struct StepperRoss : StepperBase, Ross_constants {
  typedef D Dtype;
  MatDoub dfdy;
  VecDoub dfdx;
...
StepperRoss<D>::StepperRoss(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx,
    ...
    : StepperBase(yy, dydxx, xx, atoll, rtoll, dens), dfdy(n, n), dfdx(n),
      k1(n), k2(n), k3(n), k4(n), k5(n), k6(n), cont1(n), cont2(n), cont3(n),
      cont4(n), a(n, n) {
  EPS = numeric_limits<Doub>::epsilon();
}
template <class D> void StepperRoss<D>::step(const Doub htry, D &derivs) {
  VecDoub dydxnew(n);
  Doub h = htry;
...
template <class D> void StepperRoss<D>::dy(const Doub h, D &derivs) {
  ...
  for (i = 0; i < n; i++)
    k6[i] = dydxnew[i] +
            (c51 * k1[i] + c52 * k2[i] + c53 * k3[i] + c54 * k4[i]) / h;
  alu.solve(k6, k5);
  for (i = 0; i < n; i++)
    ytemp[i] += k5[i];
  derivs(xph, ytemp, dydxnew);
  for (i = 0; i < n; i++)
    k6[i] = dydxnew[i] + (c61 * k1[i] + c62 * k2[i] + c63 * k3[i] +
                          c64 * k4[i] + c65 * k5[i]) /
                             h;
  alu.solve(k6, yerr);
  for (i = 0; i < n; i++)
...
void StepperRoss<D>::prepare_dense(const Doub h, VecDoub_I &dydxnew) {
  for (Int i = 0; i < n; i++) {
    ...
    cont2[i] = yout[i];
    cont3[i] =
        d21 * k1[i] + d22 * k2[i] + d23 * k3[i] + d24 * k4[i] + d25 * k5[i];
    cont4[i] =
        d31 * k1[i] + d32 * k2[i] + d33 * k3[i] + d34 * k4[i] + d35 * k5[i];
  }
}
template <class D>
...
template <class D> Doub StepperRoss<D>::error() {
  Doub err = 0.0, sk;
  for (Int i = 0; i < n; i++) {
...
template <class D> bool StepperRoss<D>::Controller::success(Doub err, Doub &h) {
