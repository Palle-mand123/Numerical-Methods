#ifndef _MINS_NDIM_H_
#define _MINS_NDIM_H_
#include "mins.h"
#include "nr3.h"
...
template <class T> struct Df1dim {
  const VecDoub &p;
  const VecDoub &xi;
  Int n;
  T &funcd;
...
template <class T> struct Dlinemethod {
  VecDoub p;
  VecDoub xi;
  T &func;
  Int n;
...
template <class T> struct Powell : Linemethod<T> {
  ...
  VecDoub minimize(VecDoub_I &pp, MatDoub_IO &ximat) {
    ...
    for (iter = 0;; ++iter) {
      ...
      fptt = func(ptt);
      if (fptt < fp) {
        Doub t = 2.0 * (fp - 2.0 * fret + fptt) * SQR(fp - fret - del) -
                 del * SQR(fp - fptt);
        if (t < 0.0) {
          fret = linmin();
          for (Int j = 0; j < n; j++) {
...
template <class T> struct Frprmn : Linemethod<T> {
  Int iter;
  Doub fret;
  using Linemethod<T>::func;
  using Linemethod<T>::linmin;
  ...
  using Linemethod<T>::xi;
  const Doub ftol;
  Frprmn(T &funcd, const Doub ftoll = 3.0e-8)
      : Linemethod<T>(funcd), ftol(ftoll) {}
  VecDoub minimize(VecDoub_I &pp) {
    const Int ITMAX = 200;
