...
struct Phylagglom {
  ...
  virtual void drootbranchfn(MatDoub &d, Int i, Int j, Int ni, Int nj, Doub &bi,
                             Doub &bj) = 0;
  ...
  void makethetree(const MatDoub &dist) {
    ...
    drootbranchfn(d, i, j, t[tp[i]].nel, t[tp[j]].nel, t[tp[i]].modist,
                  t[tp[j]].modist);
    t[root].ldau = tp[i];
    t[root].rdau = tp[j];
...
struct Phylo_wpgma : Phylagglom {
  ...
  void drootbranchfn(MatDoub &d, Int i, Int j, Int ni, Int nj, Doub &bi,
                     Doub &bj) {
    bi = bj = 0.5 * d[i][j];
  }
...
struct Phylo_nj : Phylagglom {
  ...
  Doub dminfn(MatDoub &d, Int i, Int j) { return d[i][j] - u[i] - u[j]; }
  ...
  Phylo_nj(const MatDoub &dist, Int fsr = -1) : Phylagglom(dist, fsr), u(n) {
    makethetree(dist);
  }
};
...
void phyl2ps(char *filename, Phylagglom &ph, MatChar str, Int extend, Doub xl,
             Doub xr, Doub yt, Doub yb) {
  Int i, j;
