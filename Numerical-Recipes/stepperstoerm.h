...
#include "nr3.h"
#include "stepperbs.h"
template <class D> struct StepperStoerm : StepperBS<D> {
  using StepperBS<D>::x;
  ...
  MatDoub ysavep;
  StepperStoerm(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx, const Doub atol,
                const Doub rtol, bool dens);
  void dy(VecDoub_I &y, const Doub htot, const Int k, VecDoub_O &yend, Int &ipt,
          D &derivs);
...
void StepperStoerm<D>::prepare_dense(const Doub h, VecDoub_I &dydxnew,
                                     VecDoub_I &ysav, VecDoub_I &scale,
  ...
  for (Int j = 1; j <= k; j++) {
    ...
    for (Int l = j; l >= 1; l--) {
      ...
      for (Int i = 0; i < neqn; i++) {
        ysave[l - 1][i] =
            ysave[l][i] + (ysave[l][i] - ysave[l - 1][i]) / factor;
        ysavep[l - 1][i] =
            ysavep[l][i] + (ysavep[l][i] - ysavep[l - 1][i]) / factor;
      }
    }
  }
  for (Int i = 0; i < neqn; i++) {
...
template <class D>
void StepperStoerm<D>::dense_interp(const Int n, VecDoub_IO &y,
                                    const Int imit) {
  Doub y0, y1, yp0, yp1, ypp0, ypp1, ydiff, ah, bh, ch, dh, eh, fh, gh, abh,
      gfh, gmf, ph0, ph1, ph2, ph3, ph4, ph5, fc1, fc2, fc3;
  VecDoub a(41);
  for (Int i = 0; i < n; i++) {
    y0 = y[i];
    y1 = y[3 * n + i];
      ...
      if (imit >= 3) {
        ...
        if (imit >= 5) {
          ...
          for (Int im = 7; im <= imit; im += 2) {
            ...
            a[im] = 64.0 * (y[(im + 6) * n + i] + fc1 * a[im - 2] -
                            fc2 * a[im - 4] + fc3 * a[im - 6]);
          }
        }
      }
    }
