...
#include "nr3.h"
#include "stepper.h"
template <class D> struct StepperBS : StepperBase {
  typedef D Dtype;
...
StepperBS<D>::StepperBS(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx,
    ...
    : StepperBase(yy, dydxx, xx, atoll, rtoll, dens), nseq(IMAXX), cost(IMAXX),
      table(KMAXX, n), dydxnew(n), coeff(IMAXX, IMAXX), errfac(2 * IMAXX + 2),
      ysave(IMAXX, n), fsave(IMAXX * (2 * IMAXX + 1), n), ipoint(IMAXX + 1),
      dens((2 * IMAXX + 5) * n) {
...
Doub StepperBS<D>::dense_out(const Int i, const Doub x, const Doub h) {
  ...
  Doub yinterp =
      dens[i] + theta * (dens[n + i] + theta1 * (dens[2 * n + i] * theta +
                                                 dens[3 * n + i] * theta1));
  if (mu < 0)
    return yinterp;
  ...
  for (Int i = 0; i < n; i++) {
    ...
    if (imit >= 2) {
      ...
      for (Int im = 4; im <= imit; im += 2) {
        ...
        a[im] =
            (y[n * (im + 4) + i] + a[im - 2] * fac1 - a[im - 4] * fac2) * 16.0;
      }
    }
...
template <class D>
void StepperBS<D>::dy(VecDoub_I &y, const Doub htot, const Int k,
...
void StepperBS<D>::prepare_dense(const Doub h, VecDoub_I &dydxnew,
                                 VecDoub_I &ysav, VecDoub_I &scale, const Int k,
                                 Doub &error) {
  mu = 2 * k - 1;
  for (Int i = 0; i < n; i++) {
    dens[i] = ysav[i];
...
template <class D> void StepperBS<D>::step(const Doub htry, D &derivs) {
        ...
        if (k == k_targ - 1 && !prev_reject && !first_step && !last_step) {
          if (err <= 1.0)
            break;
                   ...
                   SQR(nseq[k_targ] * nseq[k_targ + 1] / (nseq[0] * nseq[0]))) {
            reject = true;
            k_targ = k;
