...
struct Difeq {
  ...
  Difeq(const Int &mmm, const Int &nn, const Int &mptt, const Doub &hh,
        const Doub &cc2, const Doub &anormm, VecDoub_I &xx)
      : mm(mmm), n(nn), mpt(mptt), h(hh), c2(cc2), anorm(anormm), x(xx) {}
  ...
  void smatrix(const Int k, const Int k1, const Int k2, const Int jsf,
    ...
    if (k == k1) {
      ...
      s[0][jsf] = y[1][mpt - 1] -
                  (y[2][mpt - 1] - c2) * y[0][mpt - 1] / (2.0 * (mm + 1.0));
      s[1][3 + indexv[0]] = 1.0;
      s[1][3 + indexv[1]] = 0.0;
      s[1][3 + indexv[2]] = 0.0;
