...
Delaunay::Delaunay(vector<Point<2>> &pvec, Int options)
  ...
  pts[npts + 1] =
      Point<2>(xl - 0.5 * bigscale * delx, yl - 0.5 * bigscale * dely);
  pts[npts + 2] =
      Point<2>(xh + 0.5 * bigscale * delx, yl - 0.5 * bigscale * dely);
...
Doub Delaunay::interpolate(const Point<2> &p, const vector<Doub> &fnvals,
                           Doub defaultval) {
  Int n, i, j, k;
  Doub wgts[3];
  Int ipts[3];
  ...
  for (i = 0, j = 1, k = 2; i < 3; i++, j++, k++) {
    ...
    if (k == 3)
      k = 0;
    wgts[k] =
        (pts[ipts[j]].x[0] - pts[ipts[i]].x[0]) * (p.x[1] - pts[ipts[i]].x[1]) -
