...
template <class D> struct StepperDopr853 : StepperBase, Dopr853_constants {
  typedef D Dtype;
  VecDoub yerr2;
...
StepperDopr853<D>::StepperDopr853(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx,
    ...
    : StepperBase(yy, dydxx, xx, atoll, rtoll, dens), yerr2(n), k2(n), k3(n),
      k4(n), k5(n), k6(n), k7(n), k8(n), k9(n), k10(n), rcont1(n), rcont2(n),
      rcont3(n), rcont4(n), rcont5(n), rcont6(n), rcont7(n), rcont8(n) {
  EPS = numeric_limits<Doub>::epsilon();
}
template <class D> void StepperDopr853<D>::step(const Doub htry, D &derivs) {
  VecDoub dydxnew(n);
  Doub h = htry;
  for (;;) {
    dy(h, derivs);
...
template <class D> void StepperDopr853<D>::dy(const Doub h, D &derivs) {
  VecDoub ytemp(n);
  Int i;
  for (i = 0; i < n; i++)
...
template <class D> Doub StepperDopr853<D>::error(const Doub h) {
...
bool StepperDopr853<D>::Controller::success(const Doub err, Doub &h) {
  static const Doub beta = 0.0, alpha = 1.0 / 8.0 - beta * 0.2, safe = 0.9,
