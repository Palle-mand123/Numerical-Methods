...
#include "nr3.h"
#include "stepper.h"
template <class D> struct StepperDopr5 : StepperBase {
  typedef D Dtype;
  VecDoub k2, k3, k4, k5, k6;
  VecDoub rcont1, rcont2, rcont3, rcont4, rcont5;
  VecDoub dydxnew;
  StepperDopr5(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx, const Doub atoll,
...
StepperDopr5<D>::StepperDopr5(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx,
    ...
    : StepperBase(yy, dydxx, xx, atoll, rtoll, dens), k2(n), k3(n), k4(n),
      k5(n), k6(n), rcont1(n), rcont2(n), rcont3(n), rcont4(n), rcont5(n),
      dydxnew(n) {
...
bool StepperDopr5<D>::Controller::success(const Doub err, Doub &h) {
  static const Doub beta = 0.0, alpha = 0.2 - beta * 0.75, safe = 0.9,
...
template <class D> void StepperDopr5<D>::dy(const Doub h, D &derivs) {
...
template <class D> Doub StepperDopr5<D>::error() {
  Doub err = 0.0, sk;
  for (Int i = 0; i < n; i++) {
...
template <class D> void StepperDopr5<D>::step(const Doub htry, D &derivs) {
  Doub h = htry;
  for (;;) {
    dy(h, derivs);
    Doub err = error();
